(function registerUiRenderer(globalScope: typeof globalThis) {
  const namespace = globalScope.CCXP_LITE;
  if (!namespace) {
    return;
  }

  function createRenderer(targetDocument: Document): CcxpLiteDomRenderer {
  ...
  namespace.uiRenderer = { createRenderer };
})(globalThis);
