(function registerUiController(globalScope: typeof globalThis) {
  const namespace = globalScope.CCXP_LITE;
  if (!namespace) {
    return;
  }

  function createController(targetDocument: Document): CcxpLiteUiController {
    const AbortControllerConstructor =
      targetDocument.defaultView?.AbortController ?? globalScope.AbortController;
  ...
  namespace.uiController = { createController };
})(globalThis);
